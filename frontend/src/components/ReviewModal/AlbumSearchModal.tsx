...
export function AlbumSearchModal({ onSelect, onClose }: AlbumSearchModalProps) {
  ...
  <div className="bg-[#272730] rounded-lg p-8 shadow-lg w-[520px] relative">
        ...
        <button
          ...
          disabled={loading}
          className="w-full bg-[#6B818C] text-[#FEF4EA] py-2 rounded-full mb-4 hover:bg-[#435761]"
        >
        ...
        >
          ...
          {results.slice(0, visibleCount).map(album => (
            <div key={album.idAlbum} className="flex items-center mb-2">
              ...
              <span className="text-[#FEF4EA] max-w-[160px] break-words whitespace-normal">{album.strAlbum}</span>
              <button
                onClick={() => onSelect(album)}
                className="ml-auto mr-6 bg-[#8078a5] text-[#FEF4EA] px-5 py-1 rounded-full hover:bg-[#9a8fc1]"
              >
                Select
              </button>
            </div>
        ...
        {results.length > visibleCount && (
          <button
            ...
            className="w-full mt-2 bg-[#6B818C] text-[#FEF4EA] py-2 rounded-full hover:bg-[#435761]"
          >
            See more
          </button>
        )}
