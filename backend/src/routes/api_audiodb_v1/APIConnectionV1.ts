import express from "express";
import {
  getAlbum,
  getArtist,
  getMostLoved,
  getMusicCharts,
  getTrack,
  getTrending,
  lookupAlbum,
  lookupArtist,
  lookupTrack,
  lookupArtistByMBId,
} from "../../controllers/AudioDbController";

const router = express.Router();

// SEARCH endpoints (v1)
router.get("/v1/artist", getArtist);
router.get("/v1/album", getAlbum);
router.get("/v1/track", getTrack);

// LOOKUP endpoints (v1)
router.get("/v1/lookup/artist/:id", lookupArtist);
router.get("/v1/lookup/album/:id", lookupAlbum);
router.get("/v1/lookup/artist_mbid/:mbid", lookupArtistByMBId);
router.get("/v1/lookup/track/:id", lookupTrack);

// EXTRAS endpoints (v1)
//router.get("/v1/music-charts", getMusicCharts); nao disponivel
//router.get("/v1/most-loved", getMostLoved); nao disponivel
router.get("/v1/trending", getTrending);

export default router;
